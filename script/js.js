$(function() {
	...
	var desideNav = function(skip){
		$('.indexDiv').each(function(i) {
			if ($(window).scrollTop() >= $(this).offset().top - $(window).height() / 2) {
				$('#nav').eq(0).find('a').removeClass('aNow');
				if($('#nav').data('hover') != true || skip && $("#nav").data('scrolling') != true) {
